...
async function main() {
  ...
  for (const change of changes) {
    const contract = await zksyncEthers.getContractAt("VaultV1", change.vault, wallet);
    try {
      await contract.setStrategy(change.strategy);
      console.log("Changed strategy:", change.strategy, "for vault:", change.vault);
    } catch (error) {
      console.error("Failed to set strategy:", change.strategy, "for vault:", change.vault);
      console.error(error);
    }
